...
def image_old(pos, file="../../blueghost.gif"):
    x, y = pos
    # img = PhotoImage(file=file)
    # return _canvas.create_image(x, y, image = tkinter.PhotoImage(file=file), anchor = tkinter.NW)
    return _canvas.create_image(x, y, image=ImageTk.PhotoImage(Image.open(file)), anchor=tkinter.NW)


def image(pos, file="../../blueghost.gif", size=None, angle=0):
    global _image_cache
    x, y = pos

    if (file, angle) not in _image_cache:
        ...
        _image_cache[file, angle] = ImageTk.PhotoImage(img)

    return _canvas.create_image(x, y, image=_image_cache[file, angle], anchor=tkinter.NW)
